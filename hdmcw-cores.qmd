...
was already common, two different ideas emerged about how to run multiple threads.  A paper from 1997 [@NayfehOlukotun1997] says:
...
that naming convention faded.  As of this writing Intel makes a chip with 288 cores[^4], though i don't know if that's the most.
...
duo-quad core?).  Interesting, Apple apparently calls this _asymmetric multiprocessing_ since even though it's using the same
memory the system will allocate threads to different cores depending on their performance needs.  We saw a first hint of the multiple cores in the last episode where __ps__ command showed two processes running at
...
we can come to some conclusions in [Episode @sec-threads].
...
[^4]: [https://www.datacenterknowledge.com/servers/intel-announces-next-gen-288-core-xeon-server-processor](https://www.datacenterknowledge.com/servers/intel-announces-next-gen-288-core-xeon-server-processor)
