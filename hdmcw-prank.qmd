...
The computers were usually powered on and we could type commands into them (this was before mice and windowing systems).  The sales people would either ignore us, or watch us warily to make sure we didn't steal floppy disks.  Sometimes they’d engage us in conversation, but they didn’t want to be busy with us when somebody came in who might actually result in a commission.  What they didn’t know is that we already owned one of these computers, and we knew our way around it fairly well.  It's hard to believe in modern times, but owning a personal computer then was a rarity, sort of like owning a short-wave radio or a Geiger counter.
...
take about a second, so this number would take ~20 seconds (really, about 25 but who's counting?).  There are now system services to delay or wait, but i'm trying to duplicate what we did back in the olden days.
...
at a later point but for now see [@valsamaras2022]
...
The next section (starting with the _beep4eva_ label) is the fun part, an infinite loop that writes the BEL character to stdout[^3] forever.  This code is a
...
radically different than what happened on DOS 2.x except that there wasn't really a privileged mode.  Back then, you'd invoke an _interrupt_, which would start executing OS (or BIOS) code to do some system function, then return to where it was.  Current CPUs still have interrupts. They are a mechanism by which, as the name implies, important system functions can interrupt the execution of a program to perform some system task then return control to the program. I'll talk more about this later.
...
your settings (you need to have the "Audible Bell" on for your terminal).  On my machine it currently makes a sort of pleasant metronome sound.

This is a dumb program, and it's not the way anyone would accomplish this task now (as mentioned above there's a system call to "sleep" for a specified number of seconds, or nanoseconds).  But the way that my PC back in the 80s and my Macbook now would execute this program is in some ways surprisingly similar.  Like most computers since the 1950s, they would hold this program in memory and then fetch the instructions, fetch data into registers, put arguments in registers, execute the instructions, call operating system services.  On the other hand, the features added to make computers faster, smaller,
and more secure in the last 40 years make comparing the two computers like comparing the Wright Flyer to an F-35.  I hope this books helps us understand both the similarities and the differences.
...
[^3]: That is, standard output.  The terminal if you run this program from a command line.